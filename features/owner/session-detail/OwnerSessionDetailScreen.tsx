...
import { OwnerRosterSection } from './OwnerRosterSection'
...
interface OwnerSessionDetailScreenProps {
  id: string
  session: any
  viewerPlayer: any
  refreshing: boolean
...
export function OwnerSessionDetailScreen({
  id,
  session,
  viewerPlayer,
  refreshing,
  ...
  const processedPlayers = buildArrangementPlayers({ ...session, owner_sessions: ownerDetails })
  ...
  const previewMatch: MatchSession = {
    ...
    priceLabel: formatPrice(Number(ownerDetails.total_cost ?? session.total_cost ?? 0)),
  ...
  const handleShare = async () => {
    ...
    const shareMessage = `Mời bạn tham gia kèo ${FORMAT_LABELS[ownerDetails.format_type || 'social']} tại ${session.slot.court.name}!\n\nĐăng ký tham gia ngay tại đây: ${url}`
    ...
    try {
      ...
      console.warn('[OwnerSessionDetail] Failed to share:', error)
      try {
        await copyLink()
      } catch {}
    }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.background }}>
      <SecondaryNavbar
        ...
        onBackPress={() => router.replace('/owner/dashboard')}
        rightSlot={
      ...
      >
        ...
        {true /* TEMPORARY FOR TESTING */ && (
          <TouchableOpacity 
            onPress={() => router.push(`/owner/session/${id}/recap` as any)}
            style={{ 
        ...
        {isCheckInCompleted && !isCancelled && (
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 12 }}>
            <TouchableOpacity 
              onPress={() => router.push(`/owner/session/${id}/matches` as any)}
              style={{ 
                flex: 1,
                flexDirection: 'row', 
                alignItems: 'center', 
            ...
            <TouchableOpacity 
              onPress={() => router.push(`/owner/session/${id}/arrangement` as any)}
              style={{ 
                flex: 1,
                flexDirection: 'row', 
                alignItems: 'center', 
        ...
        <OwnerRosterSection
          ...
          sessionId={id}
          onUpdated={onRefresh}
          onArrangementPress={() => router.push(`/owner/session/${id}/arrangement` as any)}
        ...
        {!isCheckInCompleted && (
          <View style={{ marginTop: 24 }}>
            <SessionActionButtons
              ...
              onSaveArrangement={() => {}}
              leaveSession={() => {}}
              editPathname="/owner/create-session"
