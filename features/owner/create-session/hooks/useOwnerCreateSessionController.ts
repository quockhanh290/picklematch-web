...
import { 
  fetchOwnerSessionDetailForEditApi as fetchSessionDetailForEditApi, 
  createOwnerSessionApi as createSessionApi, 
  updateOwnerSessionApi as updateSessionApi 
} from '@/features/owner/api/sessionApi'
...
export function useOwnerCreateSessionController(editSessionId: string | null) {
  ...
  async function fetchHostProfile() {
    try {
      ...
      console.error('[OwnerController] Failed to fetch host profile:', err)
    }
  }
  ...
  async function hydrateOwnerSession() {
    ...
    try {
      ...
      if (data) {
        ...
        // Config
        setFormat(ownerDetails.format_type || 'social')
        setSelectedSubCourts(ownerDetails.sub_court_numbers || [1])
        setRequireResults(ownerDetails.require_results || false)
        setRequireApproval(session.require_approval || false)
        setHostIsPlaying(ownerDetails.host_is_playing !== undefined ? ownerDetails.host_is_playing : true)
        setHostGender(ownerDetails.format_metadata?.host_gender || 'male')
        setHostSkill(ownerDetails.format_metadata?.host_skill || 3.5)
        setPlayMode(ownerDetails.match_format || 'doubles')
        ...
        } else if (ownerDetails.format_metadata?.cost_per_person) {
          setCostPerPersonStr(String(ownerDetails.format_metadata.cost_per_person))
        }
  ...
  async function submit() {
    ...
    try {
      ...
      console.log('[OwnerController] Submit payload:', JSON.stringify(basePayload, null, 2))

      let finalSessionId: string | null = null
      if (isEditMode && editSessionId) {
        // ENTRY DEBUG
        console.log('[OwnerController] Starting edit submit for:', editSessionId)
        ...
        try {
          ...
          console.error('[OwnerController] Sync error:', syncErr)
        }
      } else {
