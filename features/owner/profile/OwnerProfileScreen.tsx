...
const INITIAL_PROFILE: OwnerProfile = {
  name: '',
  phone: null,
  city: null,
...
async function fetchOwnerProfileData(userId: string): Promise<OwnerProfile> {
  ...
  return {
    name: playerRes.data?.name ?? 'Chủ sân',
...
export function OwnerProfileScreen() {
  ...
  const [profile, setProfile] = useState<OwnerProfile>(INITIAL_PROFILE)
  ...
  const load = useCallback(async () => {
    ...
    const data = await fetchOwnerProfileData(userId)
    setProfile(data)
    setLoading(false)
  }, [userId])
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: PlusCircle,
      onPress: () => router.push('/owner/create-session' as any),
    },
    {
      id: 'my_courts',
      label: 'Sân của tôi',
      ...
      icon: Building2,
      onPress: () => router.push('/owner/edit-court' as any),
    },
    {
      id: 'edit_profile',
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            >
              <Text style={{ fontSize: 32, fontFamily: SCREEN_FONTS.headline, color: theme.onPrimaryContainer }}>
                {(profile.name?.charAt(0) || 'O').toUpperCase()}
              </Text>
            </View>
