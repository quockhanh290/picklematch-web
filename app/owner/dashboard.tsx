...
export default function OwnerDashboardScreen() {
  ...
  const fetchOwnerData = useCallback(async () => {
    ...
    if (courtBasic) {
      ...
      const { data: ownerSessionData, error } = await supabase
        .from('owner_sessions')
        .select(`
          *,
          session:id (
      ...
      if (!error && ownerSessionData) {
        // Flatten the structure for easier rendering
        const flattened = ownerSessionData.map(os => ({
          ...(os.session || {}),
          ...os,
  ...
  const renderSessionCard = (session: any) => {
    ...
    // Check if there are any confirmed players excluding the owner/host
    // We check both session.owner_id and our stored userId for maximum accuracy
    const hostId = session.owner_id || session.host_id || userId
          ...
          >
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <TouchableOpacity 
                  onPress={() => router.push(`/owner/court-detail/${court?.id}`)}
