...
export default function OwnerWebQuickStart() {
  ...
  const handleCreateMatch = async () => {
    setLoading(true)
    try {
      // Step A: Create or Get Owner (Simplified for Demo)
      // Note: In real app, we would use auth. Here we might create a temporary owner record
      const ownerId = gen_random_uuid() // Placeholder for demo logic
      
      // Step B: Update Court Owner
            ...
            {loading ? (
              ...
              <View style={{ marginTop: 20, gap: 12 }}>
                {courts.map(court => (
                  <Pressable 
                    key={court.id} 
                    onPress={() => router.push({ pathname: '/owner/claim-court', params: { courtId: court.id } } as any)}
      ...
      case 'finish':
        ...
        return (
          <View style={[styles.stepContainer, { alignItems: 'center', paddingTop: 40 }]}>
            ...
            <AppButton label="Về trang quản lý" onPress={() => router.replace('/owner/dashboard')} variant="secondary" style={{ marginTop: 24 }} />
          </View>
        )
    }
  }
