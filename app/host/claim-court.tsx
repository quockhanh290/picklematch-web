...
export default function ClaimCourtScreen() {
  ...
  // Registration State
  const [isHostRegistered, setIsHostRegistered] = useState<boolean | null>(null)
  ...
  async function handleClaim(courtId: string) {
    ...
    if (!error) {
      router.push({ pathname: '/host/court-config', params: { id: courtId } } as any)
    } else {
      setClaimingId(null)
    }
  }
  ...
  if (!isHostRegistered) {
    return (
      ...
      >
        ...
        <SecondaryNavbar title="ĐĂNG KÝ CHỦ SÂN" onBackPress={() => router.replace('/host/login')} />
        <ScrollView contentContainerStyle={{ padding: 24, paddingTop: 20 }}>
          ...
          <Text style={{ fontFamily: SCREEN_FONTS.headline, fontSize: 32, color: theme.onSurface, lineHeight: 38 }}>
            {STRINGS.host_flow.register_host}
          </Text>
          ...
          <View style={{ marginTop: 40, gap: 20 }}>
            <View>
              <Text style={{ fontFamily: SCREEN_FONTS.label, fontSize: 12, color: theme.primary, textTransform: 'uppercase', marginBottom: 8 }}>
                {STRINGS.host_flow.business_name}
              </Text>
              <TextInput
                value={businessName}
                onChangeText={setBusinessName}
                placeholder={STRINGS.host_flow.business_name_placeholder}
            ...
            <View>
              <Text style={{ fontFamily: SCREEN_FONTS.label, fontSize: 12, color: theme.primary, textTransform: 'uppercase', marginBottom: 8 }}>
                {STRINGS.host_flow.contact_phone}
              </Text>
              <TextInput
                value={contactPhone}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.background }}>
      <StatusBar style="dark" />
      <SecondaryNavbar 
        title={STRINGS.host_flow.claim_title}
        onBackPress={() => setIsHostRegistered(false)}
      />
      ...
      <View style={{ paddingHorizontal: 20, paddingVertical: 16 }}>
        <View style={{ 
          ...
          <TextInput
            placeholder={STRINGS.host_flow.search_court}
            placeholderTextColor={theme.outline}
            value={search}
                ...
                {claimingId === item.id ? (
                  ...
                  <>
                    <Text style={{ color: 'white', fontFamily: SCREEN_FONTS.cta, fontSize: 12 }}>
                      {STRINGS.host_flow.claim_action.toUpperCase()}
                    </Text>
