...
## Gotchas
- **`git worktree remove --force` trên worktree dưới `.claude/` đã XOÁ SẠCH `node_modules` của repo CHÍNH** (2026-08-14). Nhiều khả năng worktree dùng junction trỏ về node_modules dùng chung, và Windows xoá theo cả nội dung ở đích. Không mất gì thuộc mã nguồn (git sạch, package-lock nguyên vẹn), `npm ci` dựng lại đủ 1071 gói. **Lần sau: kiểm junction trước khi xoá worktree** (`dir /AL` hoặc kiểm `node_modules` của worktree có phải reparse point không), hoặc xoá thư mục node_modules của worktree trước rồi mới `git worktree remove`.
- **Metro/jest báo `Duplicated files or mocks` / `Haste module naming collision: picklematch-vn`** = có worktree nằm TRONG project root, mang thêm một `package.json` cùng tên. Metro quét cả `.claude/worktrees/`. Xoá worktree thừa là hết — nhưng đọc gotcha ngay trên trước khi xoá.
