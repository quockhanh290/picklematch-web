...
export function SessionListCard({
  ...
  if (startDate && isValid(startDate) && endDate && isValid(endDate)) {
    fullDateLabel = format(startDate, "EEEE, 'ng├áy' dd 'Th├íng' MM", { locale: vi })
    timeRangeLabel = `${format(startDate, 'HH:mm')} - ${format(endDate, 'HH:mm')}`
  } else {
    // Fallback to timeLabel parsing if raw dates are missing
    const [datePart, clockPart] = item.timeLabel.split('ΓÇó').map((part) => part.trim())
  ...
  return (
    ...
    >
      ...
      <View className="mt-3 flex-row items-center gap-2">
        {item.isRanked ? <MiniBadgeLight icon={Trophy} label="K├¿o t├¡nh ─æiß╗âm" tone="neutral" size="lg" /> : null}
        ...
        >
          ...
          <Text
            className="ml-1.5"
            style={{
              color: item.priceLabel === 'Miß╗àn ph├¡' ? theme.primary : theme.onSurfaceVariant,
              fontFamily: SCREEN_FONTS.label,
              fontSize: 12,
              lineHeight: 18,
            }}
          >
            {item.priceLabel}{item.priceLabel === 'Miß╗àn ph├¡' ? '' : '/ng'}
          </Text>
        </View>
      </View>
