-- Allow fast sequential starts from the same client preview batch.
-- A stale preview is still rejected if any non-start mutation changed the
-- session version after the preview was built.
