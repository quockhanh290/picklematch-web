-- Move live-mutation audit writes into the versioned mutation RPCs.
-- This keeps each live mutation to one DB roundtrip from Edge while preserving audit events.

create or replace function public.insert_live_mutation_audit_event(
  p_session_id uuid,
  p_round_no int,
  p_event_type text,
...
begin
  ...
  perform public.insert_live_mutation_audit_event(
    ...
    jsonb_build_object(
      ...
      'group_id', case when jsonb_array_length(v_rows) = 1 then v_rows -> 0 -> 'group_id' else 'null'::jsonb end,
      'checked_in_at', case when jsonb_array_length(v_rows) = 1 then v_rows -> 0 -> 'checked_in_at' else 'null'::jsonb end
    )
  );
...
begin
  ...
  perform public.insert_live_mutation_audit_event(
    ...
    jsonb_build_object(
      ...
      'player_ids', p_player_ids,
      'affected_player_ids', coalesce((select jsonb_agg(value ->> 'player_id') from jsonb_array_elements(v_rows)), '[]'::jsonb)
    )
  );
...
begin
  ...
  perform public.insert_live_mutation_audit_event(
    ...
    'roster_synced',
    jsonb_build_object(
      'requested_player_ids', p_player_ids,
      'revive_checked_out', p_revive_checked_out,
...
revoke all on function public.insert_live_mutation_audit_event(uuid, int, text, jsonb) from public, anon, authenticated;
grant execute on function public.insert_live_mutation_audit_event(uuid, int, text, jsonb) to service_role;
