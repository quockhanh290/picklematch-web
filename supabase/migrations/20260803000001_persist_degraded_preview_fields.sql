-- Persist the degraded-suggestion hint fields on session_live_matches so the host-side "Cách xử lý"
-- panel (degraded_reason + rescue_court_idxs) survives every client display path — cold app load,
-- snapshot hydration, and the sticky/committed-wins merge — instead of vanishing whenever the
-- ephemeral edge-response value is lost. Previously the persist RPC INSERTed only 8 columns and the
-- engine's degraded_reason/rescue_court_idxs/match_explanations were dropped on the round-trip, so
-- the edge (reattachDegradedPreviewFields) and client (useLiveBoard reattach) both re-derived them
-- from short-lived in-memory copies. Storing them makes the committed row authoritative.
--
-- rescue_court_idxs is board-relative and can go slightly stale between re-suggests (it is a hint,
-- so that is acceptable); it refreshes whenever the court's lineup is re-suggested (a fresh INSERT).
-- The lineup-only no-op normalization (court/team/resting) is intentionally left unchanged so a
-- board-wide rescue recompute does NOT churn sequence_no by re-persisting an unchanged lineup.

alter table public.session_live_matches
  add column if not exists degraded_reason text,
  add column if not exists rescue_court_idxs jsonb,
  add column if not exists match_explanations jsonb;

-- Re-create the base persist RPC: body verbatim from 20260722000004 except the suggested-row INSERT,
-- which now also writes degraded_reason / rescue_court_idxs / match_explanations from each p_matches
-- element (the edge already forwards them via getPreviewMatchesToPersist).
