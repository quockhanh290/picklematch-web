...
begin
  ...
  into
    v_slot_id,
    v_current_status,
    v_current_booking_status,
    v_current_court_id,
    v_current_start_time,
    v_current_end_time
  ...
  if v_current_booking_status = 'confirmed'
    and (
      p_court_id is distinct from v_current_court_id
      or p_start_time is distinct from v_current_start_time
      or p_end_time is distinct from v_current_end_time
    ) then
    raise exception 'Session already has confirmed court booking; court and schedule can not be changed';
  end if;
