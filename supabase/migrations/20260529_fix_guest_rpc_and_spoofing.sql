-- Migration: Fix guest RPC permission and spoofing vulnerability
-- Created: 2026-05-29

-- 1. Fix Issue #1: Grant execute permission to anon for the session detail view
GRANT EXECUTE ON FUNCTION public.get_session_detail_overview(uuid) TO anon;

-- 2. Fix Issue #5: Prevent spoofing by only updating if is_guest = true
...
BEGIN
  ...
  IF v_existing_id IS NOT NULL THEN
    ...
    -- FIX: Only update preferences and info if the user is actually a guest
    -- Authenticated users should update their info via their profile settings
