-- A preview generated before a live completion must not be started after that
-- completion, because player match counts/rest pressure have changed. Starts
-- from the same preview batch are still allowed while the intervening mutations
-- are starts only.
