...
--
-- Root cause: the stale-preview bug caused court suggestions to be re-fetched
-- after other courts in the same round completed. The re-fetch saw those players
-- as "free" (status no longer 'live') and picked them again. The existing
-- 'A player is already in a live match' guard only checks status='live', so it
-- missed players whose first match in the round had already completed.
--
-- Fix: after computing v_round_no, reject the start if any player already has
-- a non-cancelled, non-suggested match in that round_no.
