-- Fix: round-completion rest bookkeeping wrongly reset players who are ALREADY LIVE in the
-- next round. In async/out-of-order play a straggler match from round N can complete AFTER a
-- round N-rester has started a round N+1 match; the rester-update then set that player's
-- consecutive_rest+1 / consecutive_play=0 / opted_rest=false while they are mid-match, corrupting
-- their fairness counters and mis-prioritising them (reproduced via prod test-transaction: P9
-- rested r0, live in r1 -> completing the r0 straggler set P9 cp=0 cr=1 while P9 was playing r1).
--
-- Fix: exclude any player currently in a 'live' match from the rester-update in BOTH the complete
-- and cancel round-completion paths. A player holding a live match has not rested, regardless of
-- which logical round that match belongs to.
--
-- (Verified separately that plain under-fill rounds — 4 completed + 2 cancelled of a 6-court round
-- — already reset resters correctly, so the expected_round_matches path is NOT changed here.)
...
begin
  ...
  -- The `status = 'live'` check below (under the row lock) is the real guard.
  ...
  if v_round_complete then
    ...
    updated_resting as (
      ...
      where sps.session_id = p_session_id
        ...
        and not exists (
          select 1 from public.session_live_matches lm
          where lm.session_id = p_session_id
            and lm.status = 'live'
            and (lm.team_a ? sps.player_id::text or lm.team_b ? sps.player_id::text)
        )
      returning sps.*
    )
    select coalesce(
