-- Fix (RPC Bug 3/4): unify the payload-start round_no with the per-court scheme.
--
-- start_live_session_match_from_payload_versioned computed round_no = floor(next_sequence /
-- expected_round_matches). That diverges from replace_*'s round_no scheme and is skewed by
-- cancelled preview rows inflating sequence_no (heavy re-suggest churn could push a match into
-- a wrong/skipped round bucket), and depended on a shaky expected_round_matches fallback.
--
-- Fix: derive round_no from the target court's own last completed match (+1) — the same
-- straggler-safe, inflation-immune per-court scheme now used by replace_* (20260722000004).
-- Body otherwise verbatim from 20260722000003 (keeps the no-CAS gate + the round-play guard).
