-- Fix false-positive "Session changed" on the secondary (payload) start path.
-- Same root cause as 20260721000001 (persisted start): the whole-session
-- live_state_version CAS rejected the start on any unrelated version churn.
--
-- This path INSERTS a new match rather than flipping an existing suggested row, so its
-- version safety is the lenient "Preview is stale" block (kept intact): version churn is
-- allowed; only a DECREASE in the countable match count (a cancellation freed players the
-- preview didn't know about) is rejected. Together with the player-availability /
