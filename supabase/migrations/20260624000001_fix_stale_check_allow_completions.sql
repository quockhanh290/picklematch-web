-- Fix "Preview is stale" false-positives in multi-court sessions.
--
-- Root cause: conditions 3/4/5 in the stale guard rejected the start whenever
-- ANY match completed on another court after the preview was generated.
-- Completing a match changes live_state_version (+1) but NOT the countable count
-- (completed is still non-cancelled), so (version_diff) ≠ (countable_diff) fired.
-- Condition 5 also used preview_countable_match_count (a count) as a sequence_no
-- boundary, which breaks with cancelled matches in history (seq_no > count).
--
-- Fix: only reject when countable count decreased (matches were cancelled, freeing
-- players the engine didn't know about). Completions are safe — the player-
-- availability check below already guards against any real conflict.
--
-- Also exclude 'suggested' from the countable query to match the edge function's
-- definition (which also excludes suggested).
