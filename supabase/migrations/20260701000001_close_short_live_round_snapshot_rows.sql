-- Keep DB snapshot round synthesis aligned with the client live-cycle builder:
-- a live-match-only round is completed when all non-cancelled matches in that
-- round are completed, even if the round has fewer matches than court count.
