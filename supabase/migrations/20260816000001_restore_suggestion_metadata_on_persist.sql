-- Restore the host-facing tradeoff panel to the persist round-trip.
--
-- WHY THIS EXISTS
-- The engine attaches tradeoff_choices / recommended_tradeoff_choice (and, when the quality-cost model
-- is on, forced_tradeoff / wait_rescue_options) to each suggested match. The edge hands those payloads
-- straight to this RPC, then REPLACES its in-memory board with the rows this RPC returns
-- (session-live-matches-suggest/index.ts:1457) and serves that board to the app. So any field this
-- function does not store is not merely absent from the database — it is erased from the response too.
-- Between 2026-07-25 and 2026-08-16 that meant the panel reached the host exactly zero times: 0 of 58
-- post-persist matches in production dumps, and 0 of 1200+ persisted rows.
--
-- HOW IT BROKE, SO IT DOES NOT BREAK THE SAME WAY AGAIN
-- 20260712000002 did not write a function definition. It read the live function's TEXT, spliced the
-- column in with string replacement, re-executed it, and returned early if the column was already
-- present. The next full CREATE OR REPLACE of this function shipped the canonical source, which never
-- had the column, and silently undid the splice. Because the patch migration had already run, it never
-- ran again. Nothing failed, nothing logged, and suggestion_metadata is NOT NULL DEFAULT '{}' so every
-- row kept "carrying" the column while carrying nothing.
--
-- Two consequences for whoever edits this function next:
--   1. This is a whole, explicit definition. Do not reintroduce text-splicing migrations here.
--   2. `npm run check:rpc-markers` fails if this function stops mentioning suggestion_metadata. That
--      rule was removed once, on the reasoning that the column had "moved" to sync_live_suggestion_hints
--      — a function that did not exist until three weeks later. It is back on both writers.
--
-- WHAT IS STORED
-- Only the four panel keys, named explicitly. The pre-2026-07-25 behaviour was "every key that is not a
-- real column", which also persisted preview_live_state_version / preview_countable_match_count and the
-- quality warnings. Those are recomputed per request by the edge and their stale copies feed the
-- client's staleness comparisons, so they are deliberately NOT restored here.
--
-- The rest of this definition is byte-for-byte the function running in production on 2026-08-16
-- (captured with pg_get_functiondef, not copied from an older migration file, so the per-court round_no
-- of 20260808000001 and the degraded-field columns of 20260803000001 are carried forward intact).
...
begin
  ...
  if exists (
    ...
    -- Wording is deliberately unchanged even though the "already played" branch is gone: the client
    -- matches this string to invalidate a stale preview (useLiveBoard.ts:2905), and builds already in
    -- users' hands match the old text. Rewording here would silently break their invalidation.
  ...
  -- The no-op comparison intentionally covers lineup only. A board whose lineups are unchanged is not
  -- re-inserted, so its stored panel is left exactly as it was rather than refreshed.
  ...
  with inserted as (
    ...
    select
      ...
      -- Named keys, not "everything that is not a column". See the header note: the wider form also
      -- persisted preview_* staleness markers, and those must stay per-request.
      coalesce((
        select jsonb_object_agg(entry.key, entry.value)
        from jsonb_each(match.value) as entry
        where entry.key in (
          'tradeoff_choices',
          'recommended_tradeoff_choice',
          'forced_tradeoff',
          'wait_rescue_options'
        )
      ), '{}'::jsonb)
