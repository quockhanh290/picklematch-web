-- Relax "Preview is stale" for the case where no new matches were started since
-- the preview was generated but live_state_version advanced due to non-match
-- events (player check-in/out, opted-rest auto-clear after round end, etc.).
--
-- Previously Condition 2 used `<=`, which fired even when current_countable
-- equals preview_countable (zero new matches). This false-positive stale occurs
-- when the DB version bumps for player-state events between preview stamp time
-- and match start time. The player-availability check below (lines ~126-141)
-- already catches any real staleness caused by player state changes.
--
-- Change: `v_current_countable_match_count <= v_preview_countable_match_count`
--      → `v_current_countable_match_count < v_preview_countable_match_count`
-- Effect: only stale when countable count DECREASED (matches were cancelled),
-- not when it stayed the same (no new matches, version bumped for other reasons).
...
declare
  ...
  v_started_after_preview_count int;
...
begin
  ...
  if coalesce(p_audit_payload ->> 'source', '') = 'client-preview-start-live-match' then
    ...
    if v_preview_live_state_version <> v_live_state_version then
      ...
      select count(*)::int
      into v_started_after_preview_count
      from public.session_live_matches
      where session_id = p_session_id
        and status = 'live'
        and sequence_no >= v_preview_countable_match_count;

      if v_live_state_version < v_preview_live_state_version
        or v_current_countable_match_count < v_preview_countable_match_count
        or v_started_after_preview_count <> (v_current_countable_match_count - v_preview_countable_match_count)
