-- REVERT of 20260726000001 (rester-excludes-live-players). That fix was WRONG: resetting a
-- round-N rester's consecutive_play=0 / consecutive_rest+1 is CORRECT even when the player has
-- already started a round N+1 match — their completed play-streak did break at round N, and the
-- transient consecutive_rest=1 is harmless because the player is busy (live) and cannot be
-- re-selected. Excluding them instead LEFT consecutive_play un-reset -> inflated it (verified:
-- rester with cp=3, live next round -> stayed cp=3 instead of resetting to 0). Restore the
-- original correct round-completion bookkeeping.
