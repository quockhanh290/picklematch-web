-- Fix false-positive "Session changed" when cancelling a suggested/live match.
-- Same root cause as the start/complete gates: the whole-session live_state_version
-- CAS rejected the action whenever the version advanced for any unrelated reason.
-- The `status in ('suggested','live')` check under the `for update` row lock already
-- makes cancellation idempotent and correct; the coarse CAS only caused false rejects.
--
-- Fix: drop the `live_state_version <> p_expected_live_state_version` rejection.
-- Body copied verbatim from 20260522000003.
--
-- Contract mirrored by tests/host/startGateModel.ts::evaluateCancelGate.
