-- Fix false-positive "Session changed" when completing a live match.
-- Same root cause as the start gate (20260721000001): the whole-session
-- live_state_version CAS rejected completing court A whenever court B (or a preview
-- re-persist) advanced the version. The specific guard `status = 'live'` under the
-- `for update` row lock already makes completion idempotent and correct; the coarse
-- CAS only produced false rejections during normal rolling play.
--
-- Fix: drop the `live_state_version <> p_expected_live_state_version` rejection.
-- Everything else (round-completion bookkeeping, player-state and pair-history
-- updates, version bump) is unchanged. Body copied verbatim from 20260601000001.
--
-- Contract mirrored by tests/host/startGateModel.ts::evaluateCompleteGate.
