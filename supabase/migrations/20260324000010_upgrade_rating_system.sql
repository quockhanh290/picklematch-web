alter table public.players
  ...
  add column if not exists host_reputation integer not null default 0,
  add column if not exists earned_badges text[] not null default '{}'::text[];

alter table public.ratings
  add column if not exists skill_validation text not null default 'matched',
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'ratings_skill_validation_check'
  ) then
    alter table public.ratings
      add constraint ratings_skill_validation_check
      check (skill_validation in ('weaker', 'matched', 'outclass'));
  end if;
end $$;
...
grant execute on function public.process_final_ratings(uuid) to authenticated;
