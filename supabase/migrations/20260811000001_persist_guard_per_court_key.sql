-- BUG #5: the "already played in this round" guard keyed on v_round_no (max completed round over ALL
-- target courts + 1) while the INSERT below keys on each court's OWN next round. Two different keys, so
-- the guard was wrong in both directions once courts drifted apart:
--   court A finished round 5, court B finished round 1 -> v_round_no = 6, inserts are A=6, B=2.
--   * FALSE REJECT: a player who just finished round 6 on the fast court A is free, but seating them on
--     court B (which inserts round 2) is refused because they hold a completed row numbered 6.
--   * FALSE PASS: that same player already having a completed round 2 on court B is NOT caught, because
--     the guard only ever looked at round 6.
-- Aligning the guard to the per-court key makes the branch unreachable by construction: the insert round
-- is max(completed on that court) + 1, so a completed row cannot already carry it. The branch is dropped
-- rather than written dead.
--
-- Nothing here lets anyone play twice. These rows are inserted as 'suggested', not 'live' — persisting a
-- suggestion is not playing. The double-play case lives in the start path, and
-- start_live_session_match_from_payload_versioned keeps its own same-round guard (that one derives the
-- round per court already), covered by tests/host/live-match-start-gate.test.ts:217.
--
-- The other constraints stay untouched: nobody currently in a 'live' match, nobody holding a suggestion
-- on a court outside this batch, and nobody appearing twice in one payload (that last one is already
-- enforced earlier by the 'A player cannot appear in more than one suggested match' check).
