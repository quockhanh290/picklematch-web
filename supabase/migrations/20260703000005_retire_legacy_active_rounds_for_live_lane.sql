-- Retire legacy session_rounds.active for the live-lane flow.
-- session_rounds.completed remains valid history for older sessions, but active
-- state must come from session_live_matches in the modern court-lane flow.
...
begin
  ...
  from (
    with closable_rounds as (
      ...
      having bool_and(slm.status = 'completed')
    ),
    round_agg as (
      select
        slm.round_no,
...
begin
  raise exception 'Legacy round flow is disabled. Start individual live matches instead.';
