...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('cancel_live_session_match_versioned', {
      ...
      p_audit_payload: {
        ...auditPayload,
        source: 'session-live-matches-cancel',
      },
    })
    if (error) return jsonResponse({ ok: false, error: error.message }, 409, request)
    console.log('[session-live-matches-cancel] timing', { total: Date.now() - t0 })
    return jsonResponse({ ok: true, ...data }, 200, request)
  } catch (error) {
    return jsonResponse(
      { ok: false, error: error instanceof Error ? error.message : 'Could not cancel live match' },
      500,
      request,
    )
  }
