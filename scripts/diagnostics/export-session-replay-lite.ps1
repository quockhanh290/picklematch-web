...
$manifest.files["debug_dumps.lite.jsonl"] = Export-JsonlQuery `
  ...
  -RowExpression "jsonb_build_object('id', dd.id, 'created_at', dd.created_at, 'session_id', dd.session_id, 'decision_source', dd.decision_source, 'dump_level', coalesce(dd.payload->>'dump_level', 'legacy_full'), 'full_dump_reason', dd.payload->>'full_dump_reason', 'missing_courts', dd.missing_courts, 'missing_open_courts', coalesce(dd.payload->'missing_open_courts', dd.missing_courts), 'missing_target_courts', coalesce(dd.payload->'missing_target_courts', '[]'::jsonb), 'partial_full_board_request', coalesce((dd.payload->>'partial_full_board_request')::boolean, false), 'target_count_shortfall', coalesce((dd.payload->>'target_count_shortfall')::int, 0), 'target_expected_count', coalesce((dd.payload->>'target_expected_count')::int, null), 'filled_target_count', coalesce((dd.payload->>'filled_target_count')::int, null), 'chosen_matches_count', coalesce(jsonb_array_length(dd.chosen_matches), 0), 'rounds_count', coalesce(jsonb_array_length(dd.rounds), 0), 'payload_bytes', pg_column_size(dd.payload), 'chosen_matches_bytes', pg_column_size(dd.chosen_matches), 'rounds_bytes', pg_column_size(dd.rounds), 'payload_keys', (select coalesce(jsonb_agg(key order by key), '[]'::jsonb) from jsonb_object_keys(dd.payload) as key), 'client_request_id', dd.payload->>'client_request_id', 'suggestion_request_id', dd.payload->>'suggestion_request_id', 'current_round', coalesce(dd.payload->>'current_round', dd.payload#>>'{derived_state_summary,current_round}'), 'court_count', dd.payload->>'court_count', 'selection_debug_count', coalesce((dd.payload->>'selection_debug_count')::int, jsonb_array_length(dd.payload->'selection_debug'), 0), 'raw_payloads_before_final_board_count', coalesce((dd.payload->>'raw_payloads_before_final_board_count')::int, jsonb_array_length(dd.payload->'raw_payloads_before_final_board'), 0), 'final_preview_board_count', coalesce((dd.payload->>'final_preview_board_count')::int, jsonb_array_length(dd.payload->'final_preview_board'), 0), 'live_match_rows_count', coalesce((dd.payload#>>'{live_match_summary,total}')::int, jsonb_array_length(dd.payload->'live_match_rows'), 0), 'busy_player_ids_count', coalesce(jsonb_array_length(dd.payload->'busy_player_ids'), ((dd.payload#>>'{derived_state_summary,live_busy_players}')::int + (dd.payload#>>'{derived_state_summary,suggested_busy_players}')::int), 0))" `
