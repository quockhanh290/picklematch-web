...
import { Tier } from '../lib/next-round-suggester/classify.ts'
// @ts-ignore Node's strip-only test runner needs the local .ts extension.
import type { FairnessWarning } from '../lib/next-round-suggester/fairness/detector.ts'
// @ts-ignore Node's strip-only test runner needs the local .ts extension.
import { applyFairnessAdjustment } from '../lib/next-round-suggester/fairness/corrector.ts'
// @ts-ignore Node's strip-only test runner needs the local .ts extension.
import type { AdjustmentResult } from '../lib/next-round-suggester/fairness/corrector.ts'
// @ts-ignore Node's strip-only test runner needs the local .ts extension.
import { computeAvailabilityMetrics } from '../lib/next-round-suggester/fairness/metrics.ts'
// @ts-ignore Node's strip-only test runner needs the local .ts extension.
import type { AvailabilityMetrics } from '../lib/next-round-suggester/fairness/metrics.ts'
import {
  ...
  // @ts-ignore Node's strip-only test runner needs the local .ts extension.
} from '../lib/next-round-suggester/score.ts'
// @ts-ignore Node's strip-only test runner needs the local .ts extension.
import { bestPartitioning } from '../lib/next-round-suggester/pair.ts'
// @ts-ignore Node's strip-only test runner needs the local .ts extension.
import { suggestNextMatch, type EngineInstrumentEvent, type ExhaustiveFallbackDiagnostic } from '../lib/next-round-suggester/suggest.ts'
// @ts-ignore Node's strip-only test runner needs the local .ts extension.
import {
  chooseRollingHorizonAlternative,
  type RollingPlanTarget,
  // @ts-ignore Node's strip-only test runner needs the local .ts extension.
} from '../lib/next-round-suggester/planner/rolling-horizon.ts'
// @ts-ignore Node's strip-only test runner needs the local .ts extension.
import {
...
// @ts-ignore Node's strip-only test runner needs the local .ts extension.
import { reconstructLiveRounds } from '../lib/next-round-suggester/live-rounds.ts'
// @ts-ignore Node's strip-only test runner needs the local .ts extension.
import { getEffectivePvna } from '../lib/next-round-suggester/state.ts'
// @ts-ignore Deno edge-function bundling needs the local .ts extension.
import { computeQualityCost, jointRepartition, type Foursome } from '../lib/next-round-suggester/quality-cost.ts'
// @ts-ignore Deno edge-function bundling needs the local .ts extension.
import { isQualityCostModelEnabled } from '../lib/next-round-suggester/quality-cost-flag.ts'
// @ts-ignore Deno edge-function bundling needs the local .ts extension.
import { buildFreshestLineup, findMinCostFoursome, simulateWaitWouldClean } from '../lib/next-round-suggester/forced-tradeoff.ts'
import type {
  Match,
  PlayerSessionState,
  SessionLiveMatchRow,
...
const LIVE_PREVIEW_BATCH_TIMEOUT_MS = Number(process.env.BATCH_MS ?? 3800)
const LIVE_PREVIEW_MIN_COURT_TIMEOUT_MS = 350
const LIVE_PREVIEW_MAX_COURT_TIMEOUT_MS = Number(process.env.COURT_MS ?? 900)
...
export function repairAllIdlePayloadBatchParticipation(
  ...
  ;(globalThis as any).__ENTER__ = (globalThis as any).__ENTER__ ?? {}; (globalThis as any).__ENTER__['participation'] = ((globalThis as any).__ENTER__['participation'] ?? 0) + 1
  if (process.env.ABLATE?.split(',').includes('participation')) return payloads
  if (payloads.length < 2) return payloads
...
function repairPayloadBatchRepeatExposure(
  ...
  ;(globalThis as any).__ENTER__ = (globalThis as any).__ENTER__ ?? {}; (globalThis as any).__ENTER__['repeatExposure'] = ((globalThis as any).__ENTER__['repeatExposure'] ?? 0) + 1
  if (process.env.ABLATE?.split(',').includes('repeatExposure')) return payloads
  let current = payloads
  let currentStats = getPayloadBatchStats(current, state, pvnaTolerance)
...
export function repairPayloadBatchSevereRepeatFromPool(
  ...
  ;(globalThis as any).__ENTER__ = (globalThis as any).__ENTER__ ?? {}; (globalThis as any).__ENTER__['repeatPool'] = ((globalThis as any).__ENTER__['repeatPool'] ?? 0) + 1
  if (process.env.ABLATE?.split(',').includes('repeatPool')) return payloads
...
export function repairPayloadBatchBlowoutFromPool(
  ...
  ;(globalThis as any).__ENTER__ = (globalThis as any).__ENTER__ ?? {}; (globalThis as any).__ENTER__['blowoutPool'] = ((globalThis as any).__ENTER__['blowoutPool'] ?? 0) + 1
  if (process.env.ABLATE?.split(',').includes('blowoutPool')) return payloads
  if (payloads.length === 0 || benchIds.length === 0) return payloads
...
export function applyJointRepartition(
  ...
  ;(globalThis as any).__ENTER__ = (globalThis as any).__ENTER__ ?? {}; (globalThis as any).__ENTER__['joint'] = ((globalThis as any).__ENTER__['joint'] ?? 0) + 1
  if (process.env.ABLATE?.split(',').includes('joint')) return payloads
        ...
        try {
          options.onInstrumentEvent?.({
            ...
            detail: [
              ...
              `reject=${beamAlt.diagnostics.best_rejected_candidate_index ?? -1}`,
              `reject_delta=${beamAlt.diagnostics.best_rejected_delta?.toFixed(2) ?? 'na'}`,
              `reject_worst=${beamAlt.diagnostics.best_rejected_worst_path_score?.toFixed(2) ?? 'na'}`,
            ].join(';'),
            court_count: courtCount,
