...
export function buildSuggestedMatchPayloads({
    ...
    : applyJointRepartition(invariantSafePayloads, repairState, pvnaTolerance, onRepairInstrument)
  {
    ...
    if (g.__PASS_STATS__) {
      const key = (arr: SuggestedMatchPayload[]) => arr.map(pl => `${pl.court_idx}:${seatedLineupKey(pl.team_a, pl.team_b)}`).sort().join(',')
      const diff = (a: SuggestedMatchPayload[], b: SuggestedMatchPayload[]) => {
        const am = new Map(a.map(pl => [pl.court_idx, seatedLineupKey(pl.team_a, pl.team_b)]))
        return b.filter(pl => am.get(pl.court_idx) !== seatedLineupKey(pl.team_a, pl.team_b)).length
      }
      const stages: [string, SuggestedMatchPayload[]][] = [
        ['0_seed', payloads],
        ['1_repairBatch', repairedPayloads],
        ['2_participation', participationRepairedPayloads],
        ['3_repeatPool', repeatPoolRepairedPayloads],
        ['4_blowoutPool', blowoutPoolRepairedPayloads],
        ['5_invariantGuard', invariantSafePayloads],
        ['6_joint', jointRepartitionedPayloads],
      ]
      for (let i = 1; i < stages.length; i++) {
        const [name, cur] = stages[i]
        const prev = stages[i - 1][1]
        g.__PASS_STATS__.push({ name, changedCourts: diff(prev, cur), courts: cur.length, identical: key(prev) === key(cur) })
      }
      // Does a later pass undo an earlier one? Compare each stage back to the seed.
      for (let i = 1; i < stages.length; i++) {
        g.__PASS_BACK__ = g.__PASS_BACK__ ?? []
        g.__PASS_BACK__.push({ name: stages[i][0], vsSeed: diff(payloads, stages[i][1]) })
      }
    }
  }
