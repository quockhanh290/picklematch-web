...
| 36 | Gợi ý nhánh legacy bị dán nhãn `edge_committed` → Start với id không tồn tại | `useLiveBoard.ts:2357, 2810-2815` | CHƯA KIỂM | Nhãn gán ở hai chỗ khác bản chất: `useLiveBoard.ts:1490` hydrate hàng ĐÃ có trong DB (Start theo id chạy được), `:2724` đóng dấu lúc commit preview có thể CHƯA persist. Cần lần theo đường Start dùng id hay dùng payload mới kết luận |
