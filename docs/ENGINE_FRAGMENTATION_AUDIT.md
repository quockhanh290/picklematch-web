...
1. **Hai hệ scoring** (`score.ts` hard-gate INFINITY vs `quality-cost.ts` soft cost) sống song song trong cùng một hàm `scoreMatch`, chia nhau bằng **một dòng `if`** (score.ts:622). Flag-ON làm early-return **trước mọi hard gate**. Kiểm lại 2026-08-11: chặn rematch nguyên-4-người **đúng là mất** và đã khôi phục dưới dạng rào bậc (ALGO 69); còn tolerance thì **không mất** — rào tự triệt tiêu, xem mục Chiều 1. Cap intra 0.75/1.0 dưới cost model là **định giá mềm** (`intraTie`/`intraOver`) chứ không phải cổng cứng, và đó là chủ ý của model, chưa có bằng chứng nó gây hại. Fix ALGO 55 "within-tol-first" chỉ nằm trong `bestSplitForFoursome` — tức **chỉ phủ path joint ≥2 sân**; path live 1 sân vẫn mua được việc vượt tolerance bằng gender-pref (BUG #10, đã repro).
...
- ~~**Thang 8-stage relaxation vô hiệu hoàn toàn dưới flag ON**~~ — **KHẲNG ĐỊNH NÀY SAI, đã kiểm 2026-08-11.** Rào tolerance cộng **cùng một hằng số** cho MỌI ứng viên vượt ngưỡng nên chúng vẫn xếp hạng được với nhau: rào **tự triệt tiêu** khi không còn lựa chọn nào trong ngưỡng. Đó là lý do cost model **cố ý** không đọc cờ nới của thang cũ — đọc là buộc nó trở lại đúng cái thang nó sinh ra để thay thế. Có test pin kèm giải thích: `tests/next-round-suggester/unit/score-tolerance-barrier.test.ts`. Tôi đã thử "sửa" theo mô tả này và chính test đó bắt được.
...
| P2-2 | Gộp 7 post-pass thành 1 optimizer | **Chỉ 3 pass đổi board**: joint 60, early ~56, swap 2. **`blowoutPool` vào 2762 lần / đổi 0** (lưu ý: `entered` chỉ nghĩa là bench không rỗng, KHÔNG phải có sân cần cứu — và đào tiếp ra lỗi `'both'` bị bỏ qua, đã vá ALGO 74), `participation` 60/0, `repeatPool` 60/0 — dưới CẢ HAI model. Ba pass đó là ứng viên **XOÁ**, không phải gộp. Chờ `debug_dumps` prod xác nhận | |
...
| 9 | Board-wide pass chia chung 400ms theo thứ tự mảng → sân degraded cuối **mất "Chờ Sân X"** | `index.ts:1489-1520`, `live-preview.ts:3968` | 4, 9 | ĐÃ SỬA | live-preview.ts: `getRescueBudgetShareMs` chia suất theo số sân, sàn 80ms; bỏ hẳn bộ tích luỹ dùng chung (ALGO 72) |
...
| 21 | Rolling-horizon so điểm candidate trên số path KHÔNG bằng nhau khi hết budget | `planner/rolling-horizon.ts:572-591, 643-645` | ĐÃ SỬA | planner/rolling-horizon.ts: ứng viên bị đồng hồ cắt ngang bị loại thay vì chấm trên ít path hơn; quét 8 điểm cắt cho cùng một người thắng (ALGO 73) |
...
| 31 | `bestPartitioning` random-restart phụ thuộc wall-clock, không tie-break tổng thể | `pair.ts:751, 258` | ĐÃ SỬA | pair.ts `shouldReplaceBestPartition`: điểm vẫn quyết định, chỉ hoà tuyệt đối mới rơi xuống so board identity |
| 32 | Cổng chất lượng client áp 2 ngưỡng khác nhau cho 2 lane cùng board | `preview-consistency.ts:198-215` | ĐÃ SỬA | preview-consistency.ts: pha lấy theo `state.rounds.length` (toàn phiên) thay vì `match.round_no` per-sân |
...
| 36 | Gợi ý nhánh legacy bị dán nhãn `edge_committed` → Start với id không tồn tại | `useLiveBoard.ts:2357, 2810-2815` | ĐÃ SỬA | Nhãn KHÔNG phải thẩm quyền: `preview-consistency.ts:157-162` đòi `edge_committed` VÀ id phải thuộc batch đã commit / lane đã commit / tập id server đã persist. Không thuộc tập nào thì `useLiveBoard.ts:501-507` gọi `blockUntrustedStart('preview_not_committed')` và từ chối — không có đường Start bằng id không tồn tại |
...
| 41 | Breakpoint tính 1 lần lúc import → web resize không đổi layout | `ScreenComponents.tsx:83-86` + 7 file | ĐÃ SỬA | 8 file chuyển sang `useWindowDimensions()`; xác nhận không còn chỗ đọc kích thước lúc import |
