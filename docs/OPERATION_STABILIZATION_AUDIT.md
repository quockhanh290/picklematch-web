...
The focused-version poll stored navigation focus in React state and changed that state from `useFocusEffect` mount/cleanup. The route also omitted the optional `players` prop while the component defaulted it to a fresh `[]`, and the model allocated a fresh empty `LiveRows` object whenever query data was unavailable. Two passive recovery effects then changed state from derived request keys: one reset `startedPreviewIds`, while the next invalidated a stale batch and incremented `previewRefreshNonce`. Version-poll refetches changed the request key, those setters scheduled more renders and key changes, and the chain cycled until React raised `Maximum update depth exceeded`. Browser traces identified both setters directly at lines 2302 and 2317 in successive builds.

Fix: focus is now an imperative ref because it only gates the polling callback and has no visual output. The optional roster default and empty query rows are module-level constants, so empty inputs retain identity across renders. Both derived-key state effects and their otherwise-unused refs were removed. The single preview-fetch effect already invalidates stale batches and fetches from the current board, while started preview IDs and retry nonces retain explicit updates at operation/timer boundaries. The poll still skips work while blurred or backgrounded.
